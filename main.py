...
def get_args():
    ...
    parser.add_argument('--reference-imgs-dir', type=str, help='The path of reference images (absolute path required)')
    parser.add_argument('--video', type=str, help='Video that will process')
    parser.add_argument('--output-dir', type=str, help='The path for saving videos')
    parser.add_argument('--face-similarity-threshold', type=float, help='Threshold for minimum face similarity')
    parser.add_argument('--merge-threshold', type=float)
    parser.add_argument('--do-merge', type=parse_boolean, default="False", help='Wheather the clips will be merged or not')
...
if __name__ == "__main__":
    ...
    try:
        if not os.path.exists(reference_imgs_dir):
            raise FileNotFoundError(f"Error: Reference images directory '{reference_imgs_dir}' not found.")
        
        os.makedirs(output_dir, exist_ok=True)

        reference_imgs_embeddings = get_reference_faces_embeddings(reference_imgs_dir)
        ...
        while True:
            ...
            if len(faces) > 0:
                similarity = find_max_similarity(reference_imgs_embeddings, faces)
        ...
        print(f'Number of clips: {len(processed_clip_durations)}')
